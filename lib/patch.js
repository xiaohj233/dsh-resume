...
/**
 * Resolve one (packageName, file) target from one anchor. Returns the real
 * path of the target file, or null when the package (or its target file) is
 * not installed at this anchor. A package that IS installed but at an
 * unsupported version — or whose manifest cannot be read — refuses loudly,
...
function resolveTarget(anchor, packageName, file) {
	...
	try {
		...
		throw new Error(`dsh-resume: refusing to patch ${packageName} — its manifest is unreadable (${manifestPath}): ${error instanceof Error ? error.message : String(error)}`);
	}
	const expected = TARGET_VERSIONS[packageName];
	if (manifest.version !== expected) {
		throw new Error(`dsh-resume: refusing to patch ${packageName} — installed version "${manifest.version}" (${root}) is not the supported exact "${expected}"; update the plugin or install the supported version`);
	}
	try {
...
/** All reachable target files across anchors, deduplicated by real path. */
function resolveTargets(anchors) {
	const seen = new Set();
	const files = [];
	for (const anchor of anchors) {
		for (const [packageName, file] of TARGET_FILES) {
			const real = resolveTarget(anchor, packageName, file);
...
/** The hunk list that patches one target file (matched by package name in the path). */
function hunksForFile(file) {
	return file.includes("dsh-agent-loop") ? AGENT_LOOP_HUNKS : file.includes("dsh-session") ? SESSION_HUNKS : CONVERSATION_HUNKS;
}
 ...
 * paths are deduplicated, so a junctioned fallback adds no second patch pass.
 *
 * Guards (all enforced before ANY write):
 * - every resolved target package must be at the exact supported version;
 * - a hunk without its marker must find its `from` anchor exactly once.
 * Idempotent per hunk: a file already carrying a hunk's marker is left alone.
 ...
 * resolution (used by tests and tooling; default behavior is unchanged).
...
export function patchInstalled(extraAnchors = [], options = {}) {
	...
	const files = resolveTargets(anchors);
	/* Compute every file's patched content BEFORE writing anything: a missing
	or ambiguous anchor anywhere refuses the whole run without a single write. */
	const results = [];
	const writes = [];
	for (const file of files) {
		const source = readFileSync(file, "utf8");
		const { content, results: hunkResults } = applyHunksToSource(source, hunksForFile(file), file);
		results.push(...hunkResults);
		if (content !== source) writes.push({ file, content });
	}
	for (const { file, content } of writes) writeFileSync(file, content);
	const patched = results.filter((result) => result.status === "patched");
	const summary = patched.length === 0
		? `all ${results.length} hunk(s) over ${files.length} reachable copy/copies already carry the patch`
		: `patched ${patched.map((result) => basename(dirname(dirname(result.file))) + "/" + basename(result.file) + " (" + result.what + ")").join(", ")}`;
 ...
 * Version-guarded and atomic like {@link patchInstalled}; idempotent: a hunk
 * whose marker is already gone is skipped. Throws on a mismatched anchor (a
 * patched region someone edited since) before anything is written.
 * @param extraAnchors - additional package.json-anchored paths to resolve from.
...
export function unpatchInstalled(extraAnchors = [], options = {}) {
	...
	const files = resolveTargets(anchors);
	/* Compute every file's restored content BEFORE writing anything. */
	const results = [];
	const writes = [];
	for (const file of files) {
		const source = readFileSync(file, "utf8");
		const { content, results: hunkResults } = unpatchHunksToSource(source, hunksForFile(file), file);
		results.push(...hunkResults);
		if (content !== source) writes.push({ file, content });
	}
	for (const { file, content } of writes) writeFileSync(file, content);
	const reverted = results.filter((result) => result.status === "unpatched");
	const summary = reverted.length === 0
		? `all ${results.length} hunk(s) over ${files.length} reachable copy/copies already carry the original sources`
		: `unpatched ${reverted.map((result) => basename(dirname(dirname(result.file))) + "/" + basename(result.file) + " (" + result.what + ")").join(", ")}`;
	return { ok: true, summary, results };
}
