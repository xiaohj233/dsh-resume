...
function apply(ctx, config) {
	...
	/** Atomically persist one state payload (temp file + rename). */
	...
	async function restore() {
		...
		/* Consume the file PER SESSION, never all at once: each session that
		resumes successfully is removed from the persisted set immediately
		(atomic write), so a crash or kill mid-restore can never lose the
		remaining sessions — the next boot continues with what is left. Failed
		resumes stay recorded and are retried on the next boot. */
		const remaining = { running: [...toRestore], subagents: [...recordedSubagents] };
		const persistRemaining = () => {
			try {
				writeStateFile({ version: 1, updatedAt: Date.now(), running: remaining.running, subagents: remaining.subagents });
			} catch (error) {
				ctx.logger?.warn(`[dsh-resume] state persist failed: ${String(error)}`);
			}
		};
		if (toRestore.length > 0) {
			...
			for (const sessionId of toRestore) {
				try {
					...
					remaining.running = remaining.running.filter((id) => id !== sessionId);
					persistRemaining();
					appendLog(`[dsh-resume] restore ok: ${sessionId}`);
				} catch (error) {
					appendLog(`[dsh-resume] restore failed (kept for next boot): ${sessionId} — ${String(error)}`);
					ctx.logger?.warn(`[dsh-resume] restore of "${sessionId}" failed (kept for next boot): ${String(error)}`);
				}
			}
		}
		if (recordedSubagents.length > 0) {
			...
			stay persisted for the next boot AND are kept for the
			resume_interrupted_subagents tool's one-click retry. */
			...
			for (const sessionId of recordedSubagents) {
				try {
					...
					remaining.subagents = remaining.subagents.filter((id) => id !== sessionId);
					persistRemaining();
					appendLog(`[dsh-resume] restore ok (subagent): ${sessionId}`);
				} catch (error) {
					if (!pendingSubagents.includes(sessionId)) pendingSubagents.push(sessionId);
					appendLog(`[dsh-resume] restore failed (subagent, kept for next boot): ${sessionId} — ${String(error)}`);
					ctx.logger?.warn(`[dsh-resume] restore of subagent "${sessionId}" failed (kept for resume_interrupted_subagents and next boot): ${String(error)}`);
				}
			}
		}
	}
